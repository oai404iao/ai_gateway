...
个人使用情况、渠道状态和花费统计在 Console 请求时从 append-only `request_logs` 聚合；
排行榜由后台快照 worker 聚合，Console 请求只读取快照表。这些查询都不在数据平面请求路径中执行。
...
系统负载仍为管理员专属：`GET /console/v1/system/load`。Console 将它作为“运维”分组下独立的
`/admin/system-load` 页面提供，不再放在“统计”页面内。
