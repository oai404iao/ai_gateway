...
| `src/persistence.rs`、`src/persistence/` | 公共持久化 facade、后端中立 records/error、数据库不透明边界、SQLx repository、事务和查询；PostgreSQL row mapping 与当前运行时实现位于 `postgres/`，`sqlite.rs` 仅提供 feature-gated schema/type 基础 |
