# Console Web UI 原始设计与实施计划

> 状态：历史归档。本文保存 UI 落地前的 Radix/ESLint 方案、计划目录和原始 checkbox，
> 不能作为当前技术栈、路径或完成度依据。项目已迁移到 shadcn/ui `base-nova` + Base UI，
> 使用 oxlint；当前指南见[Console Web UI 架构与开发指南](../development/console-ui.md)。
