...
- `web/console/src/components/ui/checkbox.tsx` preserves explicit `aria-label` values instead of letting Base UI replace them with a nearby label association.
- `web/console/src/components/shared/resource-table.tsx` ignores clicks from interactive descendants, and `resource-table.test.tsx` pins checkbox selection without activating the clickable row.
- Leftover scan clean: `grep -n "radix-ui\|@radix-ui" web/console/src/components/ui/checkbox.tsx` returned no matches.

## Left alone

- Form consumers were unchanged because they use boolean checked values and single-argument change handlers compatible with Base UI.
...
- Base UI automatically associates nearby labels with its hidden input; the wrapper suppresses that fallback only when a consumer supplies an explicit accessible label.
...
- Select a checkbox inside the Catalog or Channels table and confirm the row does not open.
