...
|   |-- persistence/            # Shared records/errors/auth contracts, opaque database types, PostgreSQL repositories, and feature-gated SQLite schema/read/auth-account-registration foundation
...
cargo clippy --all-targets --features sqlite-backend # SQLite schema/type/runtime-snapshot/auth-account-registration foundation
...
cargo test --features sqlite-backend --test sqlite_registration_repository_integration # SQLite invitation codes, self-registration, and invite/reinvite/accept transactions
...
5. When SQLite schema, adapters, or repositories change, also run
   ...
   `cargo test --features sqlite-backend --test sqlite_runtime_repository_integration`, plus
    ...
    `cargo test --features sqlite-backend --test sqlite_account_repository_integration`, and
    `cargo test --features sqlite-backend --test sqlite_registration_repository_integration`. The
    feature currently validates migration/storage contracts, the runtime-snapshot read path, and
    Console login/session/profile/password/recovery/bootstrap persistence, registration invitation
    codes, self-registration, and invite/reinvite/accept transactions; runtime database selection
    through `[database].url` remains PostgreSQL-only until complete repository dispatch lands.
