...
12. **Financial facts and query logs have independent progress.** Production uses a process-unique spool, COPY ingress, then immutable `request_metering_facts` before independent settlement and log projection. Unique `request_settlements` receipts, balance/quota updates, and removal from `request_settlement_pending` commit together. Never checkpoint before COPY commit, acknowledge ingress before facts and projection persist, or recreate pending work on duplicate replay. Facts/receipts must not depend on or be deleted with `request_logs`; see `docs/development/independent-metering.md`.
